    ...
    return (
      <div className="p-6">
        ...
        <div className="bg-red-900/30 border border-red-800 rounded-xl p-6">
          <p className="text-red-300 font-semibold mb-2">Failed to load admin data</p>
          <p className="text-red-400 text-sm">{loadError}</p>
          <button onClick={loadData} className="mt-4 px-4 py-2 rounded-lg bg-coffee-700 hover:bg-coffee-600 text-white text-sm">Retry</button>
        </div>
      </div>
    )
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-2">
        {([
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 ${tab === t.key ? 'bg-coffee-700 text-white' : 'bg-coffee-900 text-coffee-400 hover:bg-coffee-800'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            <Database size={20} className="text-coffee-400" />
            <h2 className="text-lg font-semibold text-white">Database Stats</h2>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={s.label} className="bg-coffee-950 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-white">{s.value}</p>
                <p className="text-coffee-400 text-xs mt-1">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            <Server size={20} className="text-coffee-400" />
          ...
          <div className="space-y-3 mb-4">
            <input type="text" placeholder="Supabase/Firebase API URL" value={apiUrl} onChange={(e) => setApiUrl(e.target.value)}
              className="w-full bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-sm text-white placeholder-coffee-600 focus:outline-none focus:ring-2 focus:ring-coffee-500" />
          ...
          <div className="flex gap-2 mb-4">
            <button onClick={handleConfigureSync} disabled={!apiUrl}
              className="px-4 py-2 rounded-lg bg-coffee-700 hover:bg-coffee-600 disabled:bg-coffee-800 disabled:text-coffee-600 text-white text-sm font-medium">
              <Key size={14} className="inline mr-1" /> Configure</button>
            <button onClick={handleManualSync}
              className="px-4 py-2 rounded-lg bg-coffee-700 hover:bg-coffee-600 text-white text-sm font-medium">
              <RefreshCw size={14} className="inline mr-1" /> Sync Now</button>
          </div>
          {syncItems.length > 0 && (
            <div>
              <p className="text-coffee-400 text-xs mb-2">{syncItems.length} pending items</p>
              <div className="max-h-24 overflow-auto space-y-1">
                {syncItems.slice(0, 10).map((item: any) => (
                  <div key={item.id} className="text-xs text-coffee-500 bg-coffee-950 rounded px-2 py-1">{item.operation} {item.tableName} ({item.retryCount} retries)</div>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Users size={20} className="text-coffee-400" />
              <h2 className="text-lg font-semibold text-white">Staff Management</h2>
            </div>
            {isOwner && (
              <button onClick={() => setShowAddUser(true)}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-coffee-700 hover:bg-coffee-600 text-white text-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-coffee-800/50">
                    <td className="p-3 text-white">{u.name}</td>
                    <td className="p-3">
                      <span className={`text-xs font-semibold uppercase px-2 py-1 rounded ${u.role === 'owner' ? 'bg-purple-900/50 text-purple-300' : u.role === 'manager' ? 'bg-blue-900/50 text-blue-300' : 'bg-coffee-800 text-coffee-300'}`}>{u.role}</span>
                    </td>
                    <td className="p-3 text-coffee-400 text-sm">{u.createdAt}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'suppliers' && (
        <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Truck size={20} className="text-coffee-400" />
          ...
          {suppliers.length === 0 ? (
            <p className="text-coffee-500 text-sm">No suppliers added yet</p>
          ) : (
            <div className="space-y-3">
              {suppliers.map(s => (
                <div key={s.id} className="flex items-center justify-between p-3 bg-coffee-950 rounded-lg">
                  <div>
                    <p className="text-white font-medium">{s.name}</p>
                    <p className="text-coffee-400 text-xs">{s.contactName || ''} {s.phone || ''} {s.email || ''}</p>
                  </div>
      ...
      {tab === 'discounts' && (
        <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Tag size={20} className="text-coffee-400" />
          ...
          {discounts.length === 0 ? (
            <p className="text-coffee-500 text-sm">No discounts created yet</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                ...
                <tbody>
                  {discounts.map(d => (
                    <tr key={d.id} className="border-b border-coffee-800/50">
                      <td className="p-3 text-white font-mono font-bold">{d.code}</td>
                      <td className="p-3">
                        <span className={`text-xs font-semibold px-2 py-1 rounded ${d.type === 'percent' ? 'bg-blue-900/50 text-blue-300' : 'bg-emerald-900/50 text-emerald-300'}`}>{d.type}</span>
                      </td>
                      <td className="p-3 text-right text-coffee-300">{d.type === 'percent' ? `${d.value}%` : `$${d.value.toFixed(2)}`}</td>
                      <td className="p-3 text-right text-coffee-300">{d.minOrder > 0 ? `$${d.minOrder.toFixed(2)}` : '—'}</td>
                      <td className="p-3 text-right text-coffee-300">{d.uses}{d.maxUses ? `/${d.maxUses}` : ''}</td>
                      <td className="p-3 text-right text-coffee-300 text-xs">{d.expiresAt || '—'}</td>
      ...
      {tab === 'waste' && (
        <div className="space-y-6">
          <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
            <div className="flex items-center gap-3 mb-4">
              <Package size={20} className="text-coffee-400" />
              <h2 className="text-lg font-semibold text-white">Log Waste</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <select value={wasteProductId} onChange={e => setWasteProductId(e.target.value)}
                className="bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-coffee-500">
              ...
              <input type="number" step="0.1" min="0" value={wasteQty} onChange={e => setWasteQty(e.target.value)}
                placeholder="Quantity" className="bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-white placeholder-coffee-600 text-sm focus:outline-none focus:ring-2 focus:ring-coffee-500" />
              <input type="text" value={wasteReason} onChange={e => setWasteReason(e.target.value)}
                placeholder="Reason (e.g. spilled, expired)" className="bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-white placeholder-coffee-600 text-sm focus:outline-none focus:ring-2 focus:ring-coffee-500" />
          ...
          <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-5">
            <h2 className="text-lg font-semibold text-white mb-4">Waste History</h2>
            {wasteLogs.length === 0 ? (
              <p className="text-coffee-500 text-sm">No waste logged yet</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                  ...
                  <tbody>
                    {wasteLogs.slice(0, 20).map(w => (
                      <tr key={w.id} className="border-b border-coffee-800/50">
                        <td className="p-3 text-coffee-300 text-sm">{w.createdAt}</td>
                        <td className="p-3 text-white">{w.productName}</td>
                        <td className="p-3 text-right text-red-400">{w.quantity}</td>
                        <td className="p-3 text-coffee-300">{w.reason}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {showAddSupplier && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-6 w-96">
            <h2 className="text-lg font-semibold text-white mb-4">Add Supplier</h2>
            <div className="space-y-3">
      ...
      {showAddDiscount && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-coffee-900 border border-coffee-800 rounded-xl p-6 w-96">
            <h2 className="text-lg font-semibold text-white mb-4">Create Discount</h2>
            <div className="space-y-3">
              <input type="text" placeholder="Code (e.g. SUMMER20)" value={dscCode} onChange={e => setDscCode(e.target.value.toUpperCase())}
                className="w-full bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-white placeholder-coffee-600 font-mono focus:outline-none focus:ring-2 focus:ring-coffee-500" />
              <div className="flex gap-2">
                <button onClick={() => setDscType('percent')}
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${dscType === 'percent' ? 'bg-coffee-600 text-white' : 'bg-coffee-950 text-coffee-400 border border-coffee-700'}`}>Percent %</button>
                <button onClick={() => setDscType('fixed')}
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${dscType === 'fixed' ? 'bg-coffee-600 text-white' : 'bg-coffee-950 text-coffee-400 border border-coffee-700'}`}>Fixed $</button>
              </div>
              ...
              <input type="number" step="0.01" min="0" value={dscMinOrder} onChange={e => setDscMinOrder(e.target.value)}
                placeholder="Minimum order (optional)" className="w-full bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-white placeholder-coffee-600 focus:outline-none focus:ring-2 focus:ring-coffee-500" />
              <input type="number" min="0" value={dscMaxUses} onChange={e => setDscMaxUses(e.target.value)}
                placeholder="Max uses (optional)" className="w-full bg-coffee-950 border border-coffee-700 rounded-lg px-3 py-2 text-white placeholder-coffee-600 focus:outline-none focus:ring-2 focus:ring-coffee-500" />
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleAddDiscount} disabled={!dscCode.trim() || !dscValue}
                className="flex-1 py-2.5 rounded-lg bg-coffee-600 hover:bg-coffee-500 disabled:bg-coffee-800 disabled:text-coffee-600 text-white font-medium">Create</button>
            </div>
          </div>
        </div>
      )}
